...
class TransportSessionAuth:
    ...
    def is_valid_lari_controller(self, expected_secret: Optional[str] = None) -> bool:
        if self.principal_role != "LARI_CONTROLLER":
            return False
        if not isinstance(self.session_token, str) or len(self.session_token) < 32:
            return False
        if expected_secret is not None:
            return hmac.compare_digest(self.session_token, expected_secret)
        return True
...
def create_secret_session_authenticator(expected_secret: str) -> Callable[[str], Optional[TransportSessionAuth]]:
    ...
    def authenticator(token: str) -> Optional[TransportSessionAuth]:
        ...
        if hmac.compare_digest(token, expected_secret):
            return TransportSessionAuth(session_token=token, principal_role="LARI_CONTROLLER")
        return None
...
class LariControllerIngressService:
    ...
    def __init__(
        ...
        session_authenticator: Callable[[str], Optional[TransportSessionAuth]],
        authority_requester: Optional[GitHubRequester] = None,
    ):
        """Initialize with an active ControllerRelayService, mandatory session authenticator, and optional authority requester.

        FAIL CLOSED: A valid callable session_authenticator is strictly required.
        """
        if session_authenticator is None or not callable(session_authenticator):
            raise LariControllerIngressError(
                "STARTUP_FAIL_CLOSED: NO_AUTHENTICATOR_CONFIGURED. A valid session_authenticator is strictly required."
            )
    ...
    def authenticate_session(self, session_token: Optional[str]) -> bool:
        ...
        Arbitrary strings NEVER authenticate. Constant-time verification enforced by authenticator.
        """
    ...
    def publish_controller_authority(
        ...
        # 2. Enforce required authority body fields (all must be physically present, no permissive defaults)
        required_fields = [
            "authority_id",
            "issuer_controller",
            "subject_repository",
        ...
        for field in required_fields:
            if field not in body_dict or body_dict[field] is None:
                return {
                    "status": "REJECTED",
                    "error": f"Malformed authority artifact: required field '{field}' is missing",
                }
            if isinstance(body_dict[field], str) and not body_dict[field].strip():
                return {
                    "status": "REJECTED",
                    "error": f"Malformed authority artifact: required field '{field}' is empty",
                }
        ...
        prod = body_dict.get("production")
        if prod != "NO_GO":
            return {
                "status": "REJECTED",
                "error": f"Invalid production state '{prod}', must be strictly 'NO_GO'",
            }
