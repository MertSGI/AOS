...
def _validate_generation(generation: int) -> int:
    if isinstance(generation, bool):
        raise InvalidInputError("generation must not be a boolean")
...
class InMemoryCoordinationBackend:
    ...
    def __init__(self, clock_provider: Optional[Callable[[], datetime]] = None) -> None:
        ...
        # (worker_id, session_id) -> WorkerIdentity
        self._registered_workers: Dict[Tuple[str, str], WorkerIdentity] = {}
    ...
    def register_worker(self, identity: WorkerIdentity) -> None:
        ...
        key = (identity.worker_id, identity.session_id)
        with self._lock:
            existing = self._registered_workers.get(key)
            if existing is not None:
                if existing == identity:
                    # Same worker_id + same session_id + identical immutable identity => Idempotent
                    return
                else:
                    # Same worker_id + same session_id + contradictory capability_tags => Fail closed
                    raise InvalidIdentityError(
                        f"Contradictory identity registration for worker/session {key}"
                    )
            self._registered_workers[key] = identity
    ...
    def is_worker_registered(self, worker_id: str, session_id: str) -> bool:
        if not isinstance(worker_id, str) or not isinstance(session_id, str):
            return False
        with self._lock:
            return (worker_id.strip(), session_id.strip()) in self._registered_workers
    ...
    def try_claim(self, task_id: str, identity: WorkerIdentity, ttl_seconds: float) -> ClaimResult:
        ...
        with self._lock:
            key = (identity.worker_id, identity.session_id)
            registered = self._registered_workers.get(key)
            ...
            if current_lease is not None:
                ...
                if current_lease.status == LeaseStatus.ACTIVE and not is_expired:
                    if (
                        ...
                        # Same exact worker + session owns active lease => ALREADY_OWNED
                        # Stored TTL and expires_at remain unchanged
                        return ClaimResult(
                            disposition=ClaimDisposition.ALREADY_OWNED,
                            lease=current_lease,
                        )
                    else:
                        # Another active worker/session owns it => HELD_BY_OTHER
                        return ClaimResult(
    ...
    def heartbeat(
        ...
        with self._lock:
            ...
            if now >= current.expires_at:
                # Expired lease heartbeat fails safely and cannot resurrect lease
    ...
    def release(
        ...
        with self._lock:
            ...
            # Check backend clock for expiry
            if now >= current.expires_at:
                # Expired lease cannot be released as active ownership epoch
                updated_expired = LeaseSnapshot(
                    task_id=current.task_id,
                    worker_id=current.worker_id,
                    session_id=current.session_id,
