...
| Cerebras | Policy selects deprecated `llama-3.3-70b`; current public structured-output models include `gpt-oss-120b`. | Reported as a configuration risk only. This audit does not modify production policy/configuration; generic strict/local validation remains fail-closed. |
...
- Cerebras remains configured with deprecated `llama-3.3-70b`. Changing the configured model requires a separately authorized configuration change; this source/test/documentation-only audit deliberately leaves it untouched.
