...
class IndependentVerifier:
    ...
    def verify(
        ...
        project_id = str(execution_result.get("project_id") or (task.get("project_id") if task else "unknown"))
        task_id = str(execution_result.get("task_id") or (task.get("task_id") if task else "unknown"))
        gate = str(execution_result.get("gate") or (task.get("gate") if task else "unknown"))
        ...
        for c in check_list:
            if isinstance(c, dict) and "check_id" in c and "status" in c:
                ...
                # Also index without 'project:' prefix for matching
        ...
        if descriptor is not None and not required_check_ids:
        ...
        # Ensure all executed checks in execution_result passed (except known quarantine NOT_RUN)
        for c in check_list:
        ...
        # 7. Candidate Store & Artifact Integrity
        art_failed = False
        ...
        if cand_ext is not None and isinstance(cand_ext, dict):
            ...
            # If candidate directory is checkable on disk
            cand_id = cand_ext.get("candidate_id")
            if check_artifacts and candidate_store_dir and cand_id:
        ...
        # "must not permit the executor result alone to authorize canonical closure"
        # Canonical closure is authorized ONLY when ALL independent verification checks PASS.
