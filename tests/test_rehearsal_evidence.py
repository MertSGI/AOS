"""Comprehensive test suite for rehearsal evidence provenance validation (R3 Hardened)."""
...
def test_r3_harness_identity_mode_requirements(tmp_path):
    """Stage 12B.2-R3 Requirements: Explicit identity_mode required for REHEARSAL_HARNESS."""
    ...
    desc_str = "Memory harness"
    desc_sha = hashlib.sha256(desc_str.encode("utf-8")).hexdigest()

    # 1. FILE_BACKED_EXPLICIT_MODE_ACCEPTED
        ...
        steps=[
            {
                ...
                "status": "PASS",
                "evidence_references": ["ref_fb"],
            }
        ],
    )
    assert validate_rehearsal_report(fb_report, repo_root=REPO_ROOT, harness_root=harness_dir).is_valid is True

    # 2. IN_MEMORY_EXPLICIT_MODE_ACCEPTED
        ...
        steps=[
            {
                ...
                "execution_provenance": {
                    "executed": True,
                    "synthetic": True,
                    "invocation_identity": "in_mem_fn",
                    "result_summary": "OK",
                },
                "status": "PASS",
                "evidence_references": ["ref_im"],
            }
        ],
    )
    assert validate_rehearsal_report(im_report, repo_root=REPO_ROOT).is_valid is True

    # 3. HARNESS_IDENTITY_MODE_MISSING_REJECTED (Self-attack: File-backed without identity_mode)
    ambiguous_fb = create_base_report(
        top_level="FAIL",
        steps=[
            {
                "step_id": "step-ambiguous-fb",
                "claim_class": "DISPOSABLE_EXECUTION",
                "required_for_pass_candidate": True,
                "component": {
                    "origin": "REHEARSAL_HARNESS",
                    "name": "AmbiguousHarness",
                    "source_path": "my_harness.py",
                    "source_sha256": actual_harness_sha,
                },
                "execution_provenance": {
    ...
    res_amb_fb = validate_rehearsal_report(ambiguous_fb, repo_root=REPO_ROOT, harness_root=harness_dir)
    assert res_amb_fb.is_valid is False
    codes_fb = [e.code for e in res_amb_fb.errors]
    assert "SCHEMA_INVALID" in codes_fb or "HARNESS_IDENTITY_MODE_MISSING" in codes_fb

    # 4. HARNESS_IDENTITY_MODE_MISSING_REJECTED (Self-attack: In-memory without identity_mode)
    ambiguous_im = create_base_report(
        top_level="FAIL",
        steps=[
            {
                "step_id": "step-ambiguous-im",
                "claim_class": "DISPOSABLE_EXECUTION",
                "required_for_pass_candidate": True,
                "component": {
                    "origin": "REHEARSAL_HARNESS",
    ...
    res_amb_im = validate_rehearsal_report(ambiguous_im, repo_root=REPO_ROOT)
    assert res_amb_im.is_valid is False
    codes_im = [e.code for e in res_amb_im.errors]
    assert "SCHEMA_INVALID" in codes_im or "HARNESS_IDENTITY_MODE_MISSING" in codes_im

    # 5. HARNESS_IDENTITY_MODE_INVALID_REJECTED
    invalid_mode_report = create_base_report(
        top_level="FAIL",
        steps=[
            {
                "step_id": "step-inv-mode",
                "claim_class": "DISPOSABLE_EXECUTION",
                "required_for_pass_candidate": True,
                "component": {
                    "origin": "REHEARSAL_HARNESS",
                    "identity_mode": "HYBRID_UNKNOWN",
                    "name": "Harness",
                },
                "execution_provenance": {
                    "executed": True,
    ...
    res_inv = validate_rehearsal_report(invalid_mode_report, repo_root=REPO_ROOT)
    assert res_inv.is_valid is False
    assert "SCHEMA_INVALID" in [e.code for e in res_inv.errors]

    # 6. FILE_BACKED_WITH_IN_MEMORY_IDENTITY_CONTRADICTION_REJECTED
    fb_contradiction = create_base_report(
        top_level="FAIL",
        steps=[
            {
                "step_id": "step-fb-con",
    ...
    res_fb_con = validate_rehearsal_report(fb_contradiction, repo_root=REPO_ROOT, harness_root=harness_dir)
    assert res_fb_con.is_valid is False
    assert "SCHEMA_INVALID" in [e.code for e in res_fb_con.errors] or "HARNESS_IDENTITY_CONTRADICTION" in [e.code for e in res_fb_con.errors]

    # 7. IN_MEMORY_WITH_FILE_IDENTITY_CONTRADICTION_REJECTED
    im_contradiction = create_base_report(
        top_level="FAIL",
        steps=[
            {
                "step_id": "step-im-con",
    ...
    res_im_con = validate_rehearsal_report(im_contradiction, repo_root=REPO_ROOT)
    assert res_im_con.is_valid is False
    assert "SCHEMA_INVALID" in [e.code for e in res_im_con.errors] or "HARNESS_IDENTITY_CONTRADICTION" in [e.code for e in res_im_con.errors]

    # 8. FILE_BACKED_FILE_MISSING_REJECTED
    fb_missing = create_base_report(
        top_level="FAIL",
        steps=[
            {
    ...
    res_missing = validate_rehearsal_report(fb_missing, repo_root=REPO_ROOT, harness_root=harness_dir)
    assert res_missing.is_valid is False
    assert "HARNESS_FILE_MISSING" in [e.code for e in res_missing.errors]

    # 9. FILE_BACKED_HASH_MISMATCH_REJECTED
    fb_bad_hash = create_base_report(
        top_level="FAIL",
        steps=[
            {
    ...
    res_bad_hash = validate_rehearsal_report(fb_bad_hash, repo_root=REPO_ROOT, harness_root=harness_dir)
    assert res_bad_hash.is_valid is False
    assert "HARNESS_FILE_HASH_MISMATCH" in [e.code for e in res_bad_hash.errors]

    # 10. IN_MEMORY_HASH_MISMATCH_REJECTED
    im_bad_hash = create_base_report(
        top_level="FAIL",
        steps=[
            {
                "step_id": "step-im-bad-hash",
                "claim_class": "DISPOSABLE_EXECUTION",
                "required_for_pass_candidate": True,
                "component": {
                    "origin": "REHEARSAL_HARNESS",
    ...
    res_im_bad_hash = validate_rehearsal_report(im_bad_hash, repo_root=REPO_ROOT)
    assert res_im_bad_hash.is_valid is False
    assert "IN_MEMORY_HARNESS_HASH_MISMATCH" in [e.code for e in res_im_bad_hash.errors]
...
def test_execution_pass_evidence_requirements():
    """Execution claim classes PASS requires executed=true, invocation_identity, result_summary, evidence_references."""
    base_step = {
        "step_id": "step-exec",
...
def test_authority_decision_provenance_requirements():
    """AUTHORITY_DECISION PASS requires AOS_CORE origin, executed=true, evidence references, matching decisions."""
    auth_step = {
        "step_id": "step-auth",
        "claim_class": "AUTHORITY_DECISION",
        "required_for_pass_candidate": True,
